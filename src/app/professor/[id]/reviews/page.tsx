...
import { getProfessorReviews, getProfessor, type ProfessorReviews, type ProfessorDetail } from '@/lib/api';
import { Star, ArrowLeft, MessageSquare, Filter, ChevronLeft, ChevronRight, ThumbsUp, ThumbsDown, Loader2, AlertCircle, Calendar } from 'lucide-react';
...
export default function ProfessorReviewsPage({ params }: ProfessorReviewsPageProps) {
                ...
                <main className="pt-24 pb-20">
                    <div className="max-w-7xl mx-auto px-6">
                        <div className="text-center py-12">
                            ...
                            <p className="text-text-body mb-4">{error || 'Reviews could not be loaded'}</p>
                            <Link href="/courses">
    ...
    const availableCourses = [...new Set(professor.courses?.map(c => c.course_id).filter(Boolean) || [])];
                ...
                <div className="max-w-7xl mx-auto px-6">
                    ...
                    <div className="mb-6">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <Link href={`/professor/${professor.id}`}>
                                <Button variant="ghost" className="text-white hover:bg-[#500000]/10">
                                    {professor.name}
                                </Button>
                            </Link>
                        </div>
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-6 text-md">
                                        <div className="flex items-center gap-2 bg-gradient-to-r from-yellow-400 to-orange-400 text-white px-4 py-1 rounded-full shadow-md">
                                            ...
                                            <span className="font-semibold">{reviewsData.summary.avg_overall.toFixed(1)}</span>
                                        </div>
                                        <div className="flex items-center gap-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white px-4 py-1 rounded-full shadow-md">
                                            ...
                                            <span className="font-semibold">{reviewsData.summary.total_reviews} total reviews</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <Card className="bg-card border-border p-8 mb-8">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h3 className="text-2xl font-bold text-text-heading">Filter Reviews</h3>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                            <div>
                                <label className="text-sm font-semibold text-text-heading mb-3 block uppercase tracking-wider">Course</label>
                            ...
                            <div>
                                <label className="text-sm font-semibold text-text-heading mb-3 block uppercase tracking-wider">Sort By</label>
                                <Select value={sortBy} onValueChange={(value: 'date' | 'rating' | 'course') => setSortBy(value)}>
                                    <SelectTrigger className="bg-canvas border-border h-12">
                                        <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent className="bg-canvas border-border">
                            ...
                            <div>
                                <label className="text-sm font-semibold text-text-heading mb-3 block uppercase tracking-wider">Min Rating</label>
                                <Select value={minRating?.toString() || 'none'} onValueChange={(value) => setMinRating(value === 'none' ? undefined : Number(value))}>
                                    <SelectTrigger className="bg-canvas border-border h-12">
                    ...
                    <div className="space-y-4 mb-8">
                        {reviewsData.reviews.map((review) => (
                            <Card key={review.id} className="bg-card border-border hover:border-[#500000] hover:scale-101 transition-all duration-300 hover:shadow-lg/20">
                                <CardContent className="p-4">
                                    <div className="flex items-start justify-between mb-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <Badge variant="outline" className="bg-gradient-to-r from-[#500000] to-[#600000] text-white border-transparent px-2 py-1 text-xs font-medium">
                                                {review.course.code}
                                            </Badge>
                                            <Badge variant="outline" className="bg-gradient-to-r from-green-500 to-green-600 text-white border-transparent px-2 py-1 text-xs font-medium">
                                                Grade: {review.grade}
                                            </Badge>
                                        </div>
                                        <div className="flex items-center gap-1 text-text-body px-2 py-1 rounded-full text-xs">
                                            <Calendar className="w-3 h-3" />
                                            <span className="font-medium">{formatDate(review.review_date)}</span>
                                        </div>
                                    </div>

                                    <div className="grid grid-cols-4 gap-3 mb-4">
                                        <div className="text-center border-2 border-blue-500 bg-blue-400/10 p-2 rounded-lg">
                                            <div className="text-lg font-bold text-white">{review.clarity_rating.toFixed(1)}</div>
                                            <div className="text-xs font-medium text-white">Clarity</div>
                                        </div>
                                        <div className="text-center border-2 border-red-500 bg-red-400/10 p-2 rounded-lg">
                                            <div className="text-lg font-bold text-white">{review.difficulty_rating.toFixed(1)}</div>
                                            <div className="text-xs font-medium text-white">Difficulty</div>
                                        </div>
                                        <div className="text-center border-2 border-green-500 bg-green-400/10 p-2 rounded-lg">
                                            <div className="text-lg font-bold text-white">{review.helpful_rating.toFixed(1)}</div>
                                            <div className="text-xs font-medium text-white">Helpfulness</div>
                                        </div>
                                        <div className="text-center border-2 border-purple-500 bg-purple-400/10 p-2 rounded-lg">
                                            ...
                                            <div className="text-xs font-medium text-white">Take Again</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex flex-wrap gap-1">
                                            {review.tags?.slice(0, 3).map((tag, index) => (
                                                <Badge key={index} variant="outline" className="bg-gradient-to-r from-gray-500 to-gray-700 text-white border-transparent px-2 py-0.5 text-xs font-medium">
                                                    {tag}
                                                </Badge>
                                            ))}
                                        </div>
                                        ...
                                        <div className="flex items-center gap-3 text-text-body">
                                            <div className="flex items-center gap-1 bg-green-100 px-2 py-1 rounded-full">
                                                ...
                                                <span className="font-medium text-green-600 text-xs">{review.thumbs_up}</span>
                                            </div>
                                            <div className="flex items-center gap-1 bg-red-100 px-2 py-1 rounded-full">
                                                <ThumbsDown className="w-3 h-3 text-red-600" />
                                                <span className="font-medium text-red-600 text-xs">{review.thumbs_down}</span>
                                            </div>
                                        </div>
                                    </div>
                                </CardContent>
