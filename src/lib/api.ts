...
export async function getCourseReviews(
    ...
    const url = new URL(`${API_BASE_URL}/course/${courseId}/reviews/${professorId}`);
...
// Health Check API
export async function getHealthCheck(): Promise<{ status: string; database: any; api_version: string }> {
    try {
...
export async function compareCourses(courseIds: string[]): Promise<CourseDetail[]> {
    try {
        ...
        if (!response.ok) {
            console.warn('Course comparison API not available, falling back to individual course fetches');
            // Fallback: fetch individual courses
            const coursePromises = courseIds.map(id => getCourseDetail(id.replace(/\s+/g, '')));
        ...
        console.warn('Course comparison API failed, falling back to individual course fetches:', error);
        // Fallback: fetch individual courses
        const coursePromises = courseIds.map(id => getCourseDetail(id.replace(/\s+/g, '')));
        return Promise.all(coursePromises);
    }
...
export async function getProfessors(params?: {
    ...
    if (params?.min_rating) url.searchParams.append('min_rating', params.min_rating.toString());
    if (params?.limit) url.searchParams.append('limit', params.limit.toString());
    if (params?.skip) url.searchParams.append('skip', params.skip.toString());
...
export async function getProfessor(professorId: string): Promise<ProfessorDetail> {
    try {
        console.log('Fetching professor from URL:', `${API_BASE_URL}/professor/${professorId}`);
        const response = await fetch(`${API_BASE_URL}/professor/${professorId}`);

        if (!response.ok) {
            const errorText = await response.text();
            console.error('Professor API error:', response.status, response.statusText, errorText);
            throw new Error(`Failed to fetch professor: ${response.status} - ${errorText}`);
        }
...
export async function getProfessorReviews(
    ...
    try {
        ...
        if (params?.course_filter) url.searchParams.append('course_filter', params.course_filter);
        if (params?.limit) url.searchParams.append('limit', params.limit.toString());
        if (params?.skip) url.searchParams.append('skip', params.skip.toString());
        if (params?.sort_by) url.searchParams.append('sort_by', params.sort_by);
        if (params?.min_rating) url.searchParams.append('min_rating', params.min_rating.toString());
        if (params?.max_rating) url.searchParams.append('max_rating', params.max_rating.toString());
        ...
        if (!response.ok) {
            const errorText = await response.text();
            console.error('Professor reviews API error:', response.status, response.statusText, errorText);
            throw new Error(`Failed to fetch professor reviews: ${response.status} - ${errorText}`);
        }
        ...
        console.log('Raw professor reviews response sample:', data.length ? data.slice(0, 2) : data);
        ...
        if (Array.isArray(data)) {
            ...
            const avgOverall = totalReviews > 0 ? reviews.reduce((sum, r) => sum + (r.overall_rating || 0), 0) / totalReviews : 0;
            const avgClarity = totalReviews > 0 ? reviews.reduce((sum, r) => sum + (r.clarity_rating || 0), 0) / totalReviews : 0;
            const avgDifficulty = totalReviews > 0 ? reviews.reduce((sum, r) => sum + (r.difficulty_rating || 0), 0) / totalReviews : 0;
            const avgHelpfulness = totalReviews > 0 ? reviews.reduce((sum, r) => sum + (r.helpful_rating || 0), 0) / totalReviews : 0;
            const wouldTakeAgainCount = reviews.filter(r => r.would_take_again === true).length;
            const wouldTakeAgainPercent = totalReviews > 0 ? (wouldTakeAgainCount / totalReviews) * 100 : 0;
        ...
        if (data && typeof data === 'object' && data.reviews && Array.isArray(data.reviews)) {
...
export async function searchProfessors(params: {
    ...
    if (params.department) url.searchParams.append('department', params.department);
    if (params.min_rating) url.searchParams.append('min_rating', params.min_rating.toString());
    if (params.courses_taught) url.searchParams.append('courses_taught', params.courses_taught);
    if (params.limit) url.searchParams.append('limit', params.limit.toString());
    if (params.skip) url.searchParams.append('skip', params.skip.toString());
